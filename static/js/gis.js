(function () {
    ...
    async function zoomToLayerFeatures(layerId) {
        ...
        try {
            ...
            const response = await fetch(`/layer-feature-bounds/?${params}`);
            ...
            if (data.bounds) {
                ...
                state.map.flyToBounds(featureBounds, {
                    padding: [50, 50],
                    duration: 1.5,
                    maxZoom: 16  // Don't zoom in too close
                });

                // Show feature count message
                if (data.feature_count > 0) {
                    showMessage(`Found ${data.feature_count} features`, 'success');
                }
            } else if (data.center) {
                // No features in view, zoom to center point
                state.map.flyTo([data.center.lat, data.center.lng], 15, {
                    duration: 1.5
                });

                if (data.feature_count === 0) {
                    showMessage('No features in current view. Try zooming out.', 'info');
                }
            }
        } catch (error) {
    ...
    async function toggleLayer(layerId) {
        ...
        if (isChecked) {
            ...
            // Auto-zoom to layer features
            await zoomToLayerFeatures(layerId);
        } else {
    ...
    function showMessage(text, type = 'info') {
        ...
        // Style based on type
        if (type === 'success') {
            message.style.background = 'linear-gradient(135deg, rgba(16, 185, 129, 0.95) 0%, rgba(5, 150, 105, 0.95) 100%)';
            message.style.color = 'white';
        } else if (type === 'info') {
            message.style.background = 'linear-gradient(135deg, rgba(59, 130, 246, 0.95) 0%, rgba(37, 99, 235, 0.95) 100%)';
            message.style.color = 'white';
        }
...
// Update the zoomToLayer function to use the new approach
    function zoomToLayer(layerId) {
        ...
        if (!state.activeLayers[layerId]) {
            ...
            if (checkbox && !checkbox.checked) {
                ...
                toggleLayer(layerId);  // This will auto-zoom
                return;
            }
        }

        // If already active, just zoom to features
        zoomToLayerFeatures(layerId);
    }
    ...
    window.gisApp = {
        ...
        zoomToLayerFeatures: zoomToLayerFeatures,
