(function() {
  ...
  const CONFIG = {
    ...
    URLS: {
      ...
      TOGGLE_FAVORITE: '/api/preferences/toggle-favorite/',
      SAVE_SELECTION: '/api/preferences/save/',
      CLEAR_PREFERENCES: '/api/preferences/clear/'
    }
  };
  ...
  function init() {
    ...
    checkAuthentication().then(isAuthenticated => {
      ...
      // Load user preferences first, then layers
  ...
  async function loadUserPreferences() {
    """Load user's saved layer preferences."""
    try {
      const response = await fetch(CONFIG.URLS.GET_PREFERENCES);
      if (!response.ok) {
      ...
      state.userPreferences = {
        preferredLayerIds: data.preferred_layer_ids || [],
        favoriteLayerIds: data.favorite_layer_ids || [],
        loaded: true
      };

      console.log(`Loaded ${state.userPreferences.preferredLayerIds.length} preferred layers`);
      
      // Show notification if there are preferred layers
      if (state.userPreferences.preferredLayerIds.length > 0) {
        showPreferenceNotification(state.userPreferences.preferredLayerIds.length);
      }

    } catch (error) {
      console.error('Error loading preferences:', error);
      state.userPreferences.loaded = true; // Mark as loaded even on error
    }
  }

  function showPreferenceNotification(count) {
    """Show a toast notification about loaded preferences."""
    // Create notification element if it doesn't exist
    let notification = document.getElementById('preferenceNotification');
    if (!notification) {
      notification = document.createElement('div');
      notification.id = 'preferenceNotification';
    ...
    notification.innerHTML = `
      <span>⭐</span>
      <span>${count} preferred layer${count > 1 ? 's' : ''} ready to select</span>
      <button onclick="gisApp.selectPreferredLayers()" style="
  ...
  async function selectPreferredLayers() {
    """Select all user's preferred layers."""
    const preferredIds = state.userPreferences.preferredLayerIds;
    if (!preferredIds.length) {
      console.log('No preferred layers to select');
      return;
    }

    console.log(`Selecting ${preferredIds.length} preferred layers...`);

    // Select each preferred layer (limit to first 5 to avoid overwhelming)
    const toSelect = preferredIds.slice(0, 5);
    for (const layerId of toSelect) {
    ...
    // Hide notification
    const notification = document.getElementById('preferenceNotification');
    if (notification) {
      notification.style.display = 'none';
    }

    // Show confirmation
    showToast(`Loaded ${toSelect.length} preferred layers`);
  }

  async function toggleFavorite(layerId) {
    """Toggle favorite status for a layer."""
    try {
      const response = await fetch(CONFIG.URLS.TOGGLE_FAVORITE, {
        method: 'POST',
        headers: {
      ...
      // Update local state
      if (data.is_favorite) {
        if (!state.userPreferences.favoriteLayerIds.includes(layerId)) {
          state.userPreferences.favoriteLayerIds.push(layerId);
        }
      } else {
        state.userPreferences.favoriteLayerIds = 
          state.userPreferences.favoriteLayerIds.filter(id => id !== layerId);
      }

      // Update UI
      updateFavoriteButton(layerId, data.is_favorite);
  ...
  function updateFavoriteButton(layerId, isFavorite) {
    """Update the favorite button UI for a layer."""
    const btn = document.querySelector(`.btn-favorite[data-layer-id="${layerId}"]`);
    if (btn) {
  ...
  // ============ END USER PREFERENCES FUNCTIONS ============
  ...
  function renderLayerList() {
    ...
    // Add preferred layers section if there are any
    const preferredLayers = state.layers.filter(l => 
      state.userPreferences.preferredLayerIds.includes(l.id)
    );
    
    if (preferredLayers.length > 0) {
      html += `<div style="margin-bottom: 20px;">
        <div style="font-size: 12px; font-weight: 600; color: rgba(255, 255, 255, 0.9); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 6px;">
          <span>⭐</span> Your Preferred Layers (${preferredLayers.length})
        </div>`;
      
      preferredLayers.slice(0, 5).forEach(layer => {
        html += renderLayerItem(layer, true);
      });
      
      html += '</div><hr style="border-color: rgba(255,255,255,0.2); margin: 16px 0;">';
    }
    
    // Render all layers grouped by server
    Object.entries(grouped).forEach(([serverName, layers]) => {
      ...
      layers.forEach(layer => {
        html += renderLayerItem(layer, false);
      });
  ...
  function renderLayerItem(layer, isPreferred) {
    const isActive = state.activeLayers[layer.id];
    const isFavorite = state.userPreferences.favoriteLayerIds.includes(layer.id);
    const typeIcon = getTypeIcon(layer.type);

    return `
      <div class="layer-item ${isActive ? 'active' : ''} ${isPreferred ? 'preferred' : ''}" 
        ...
        <div class="layer-info">
          <div class="layer-name">
            ${isPreferred ? '<span style="color: #fbbf24;">★</span> ' : ''}
            ${layer.name}
          </div>
        ...
        <button class="btn-favorite ${isFavorite ? 'is-favorite' : ''}" 
                data-layer-id="${layer.id}"
                onclick="event.stopPropagation(); gisApp.toggleFavorite(${layer.id})" 
                title="${isFavorite ? 'Remove from favorites' : 'Add to favorites'}"
                style="background: transparent; border: none; cursor: pointer; font-size: 16px; padding: 4px;">
          ${isFavorite ? '⭐' : '☆'}
        </button>
  ...
  async function showNearbyLayers(lat, lng) {
    ...
    try {
      ...
      layers.forEach(layer => {
        ...
        const isPreferred = state.userPreferences.preferredLayerIds.includes(layer.id);
        
        html += `
          <div class="nearby-layer-item ${isPreferred ? 'preferred' : ''}">
            <label>
  ...
  async function exportAsDXF(layerIds) {
    ...
    // Show success message - preferences are saved automatically on server
    showToast('Export complete! Preferences saved.');
  }
  ...
  async function downloadNearbyLayers(e) {
    ...
    try {
      ...
      closeSidebar();
      showToast('Download complete! Preferences saved.');
  ...
  window.gisApp = {
    ...
    selectPreferredLayers: selectPreferredLayers
  };
